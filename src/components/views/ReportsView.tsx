...
export default function ReportsView() {
  ...
  const KPICard = ({
    ...
    <div
      className={`bg-surface rounded-xl border-2 ${color} p-5 flex flex-col gap-3 card-shadow`}
    >
      ...
      <span className="font-mono text-4xl font-black text-primary leading-none">
        {value}
      </span>
      {pct !== null && (
        <div
          className={`flex items-center gap-1 text-xs font-bold ${pct >= 0 ? "text-emerald-600" : "text-error"}`}
        >
    ...
    <div className="p-8 flex-1 flex flex-col gap-6 max-w-[1600px] mx-auto w-full">
      ...
      {error && (
        <div className="bg-error-container text-on-error-container p-4 rounded flex items-center gap-3 border border-error text-sm">
          ...
          <button onClick={cargarDatos} className="ml-auto font-bold underline">
            Reintentar
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {loading || !resumen ? (
          [1, 2, 3].map(i => (
            <div
              key={i}
              className="bg-surface rounded-xl border border-outline-variant p-5 h-36 animate-pulse"
            />
          ))
        ) : (
          <>
              ...
              {loading ? (
                ...
                pedidosPagina.map(p => (
                  ...
                  >
                    ...
                    <td className="p-4 text-center capitalize text-sm text-on-surface-variant">
                      {p.tipo}
                    </td>
                    <td className="p-4 text-right font-mono font-bold text-secondary-container">
                      ${p.total.toFixed(2)}
                    </td>
                    <td className="p-4">
                      <BadgeEstatus estatus={p.Estatus_Orden} />
                    </td>
