...
function BadgeEstatus({ estatus }: { estatus: EstatusOrden }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded ${estilos[estatus]}`}>
      {estatus === 'Preparando' ? (
...
function BadgeTipo({ tipo }: { tipo: Orden['tipo'] }) {
  const icons: Record<Orden['tipo'], string> = { mesa: '🪑', pickup: '🛍️', delivery: '🛵' };
  const labels: Record<Orden['tipo'], string> = { mesa: 'Mesa', pickup: 'Para Llevar', delivery: 'Delivery' };
  return (
...
export default function OrdersView() {
  ...
  const cancelarOrden = async (orden: Orden) => {
    if (!confirm(`¿Cancelar el pedido #${orden.id_pedido} de ${orden.cliente_nombre}? Esta acción no se puede deshacer.`)) return;
    ...
    try {
      ...
      showToast('error', 'Error al cancelar', e instanceof Error ? e.message : 'Intente nuevamente');
    } finally {
      setCancelando(null);
    }
  };
    ...
    <div className="p-8 flex-1 flex flex-col gap-6 max-w-[1200px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {(['Recibido', 'Preparando', 'Listo'] as EstatusOrden[]).map((e) => {
          ...
          const activeColors: Record<EstatusOrden, string> = {
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="font-bold text-on-surface">{orden.cliente_nombre}</span>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="font-mono font-bold text-primary">${orden.total.toFixed(2)}</span>
                  ...
                  <div className="border-t border-outline-variant p-5 bg-surface-container-low fade-in">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-5">
                      {orden.cliente_cedula && (
                        <div>
                          <span className="text-on-surface-variant font-bold block text-xs uppercase">Cédula</span>
                          <span className="font-mono">{orden.cliente_cedula}</span>
                        </div>
                      )}
                      {orden.cliente_telefono && (
                        <div>
                          <span className="text-on-surface-variant font-bold block text-xs uppercase">Teléfono</span>
                          <span className="font-mono">{orden.cliente_telefono}</span>
                        </div>
                      )}
                      {orden.tipo === 'mesa' && orden.mesa && (
                        <div>
                          <span className="text-on-surface-variant font-bold block text-xs uppercase">Mesa</span>
                          <span className="font-bold text-2xl">{orden.mesa}</span>
                        </div>
                      )}
                      {orden.tipo === 'delivery' && orden.direccion && (
                        <div className="col-span-2">
                          <span className="text-on-surface-variant font-bold block text-xs uppercase">Dirección</span>
